                    ...
                    <div className="flex gap-0.5 mb-1">
                        {stars.map((star, idx) => (
                            ...
                            >
                                <Star 
                                    size={12} 
                                    className={`${score >= star ? "fill-[#FBBC05] text-[#FBBC05]" : "fill-gray-400 text-gray-400"}`} 
                                    strokeWidth={0}
                                />
                            </motion.div>
                        ))}
...
const VideoTourCard = ({ video, poster }: { video: string, poster?: string }) => {
    ...
    const isInView = useInView(containerRef, { amount: 0.6, once: false }); // Trigger when 60% visible
    ...
    return (
        <div ref={containerRef} className="relative w-full h-[500px] md:h-[600px] rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-[0_20px_50px_-10px_rgba(0,0,0,0.3)] border border-white/10 group bg-black">
             ...
             <div className="absolute top-6 right-6 flex items-center gap-3 pointer-events-none">
                 <div className="bg-red-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider shadow-lg flex items-center gap-2">
                     <span className="w-2 h-2 rounded-full bg-white animate-pulse"></span> Live Tour
                 </div>
             </div>
             ...
             <button 
                onClick={toggleMute}
                className="absolute bottom-6 right-6 p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-white/20 transition-all z-20 group/btn"
             >
...
const GalleryCard: React.FC<GalleryCardProps> = ({ title, category, images, description }) => {
    const [current, setCurrent] = useState(0);
    const count = images && images.length > 0 ? images.length : 0;
                ...
                <AnimatePresence mode='wait'>
                    {count > 0 ? (
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-800 text-gray-500">
                            ...
                            <span className="text-xs uppercase tracking-widest font-bold opacity-50">Image à venir</span>
                        </div>
                    )}
                </AnimatePresence>
...
export const Home = () => {
  ...
  // Split gallery into video and images
    ...
    <div className="flex flex-col min-h-screen bg-[#FDFBF9]">
      ...
      {sections.hero && (
        <div className="relative h-[100dvh] w-full overflow-hidden bg-gray-900">
            {/* Stunning Rating Component */}
            <StunningRating 
                score={content.ratingScore || 4.5} 
                label={t.home.rating} 
                subLabel={t.home.onGoogle}
            />
            ...
            >
                ...
                <span className="inline-flex items-center gap-2 py-2 px-6 border border-white/30 rounded-full text-white text-xs md:text-sm font-bold tracking-[0.2em] uppercase backdrop-blur-md bg-white/10 shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:bg-white/20 transition-all cursor-default">
                    <Star size={12} className="text-primary-500 fill-primary-500" /> {t.home.welcome}
                </span>
                </motion.div>
                ...
                <Link to="/rooms" className="w-full sm:w-auto flex justify-center items-center gap-3 text-white px-10 md:px-12 py-5 md:py-6 rounded-full text-lg font-bold transition-all duration-300 hover:bg-white/10 backdrop-blur-md border border-white/30 hover:border-white group">
                    {t.home.discoverSuites}
                </Link>
                </motion.div>
            </motion.div>
      ...
      {sections.features && (
        <div className="relative z-30 -mt-16 px-4 sm:px-6 lg:px-8">
            ...
            className="max-w-7xl mx-auto bg-white/90 backdrop-blur-2xl shadow-[0_30px_80px_-20px_rgba(0,0,0,0.15)] rounded-[2.5rem] p-6 md:p-12 border border-white"
            >
            <motion.div 
                variants={staggerContainer}
                className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center divide-y md:divide-y-0 md:divide-x divide-gray-200/60"
            >
                {[
                { icon: MapPin, title: t.home.features.location, desc: t.home.features.locationDesc },
                { icon: Star, title: t.home.features.charm, desc: t.home.features.charmDesc },
                { icon: Wifi, title: t.home.features.services, desc: t.home.features.servicesDesc }
                ].map((item, idx) => (
                <motion.div variants={featureCardVariant} key={idx} className="flex flex-col items-center p-4 group cursor-default transition-all duration-300 pt-8 md:pt-4">
                    <div className="w-20 h-20 flex items-center justify-center bg-gray-50 text-gray-900 border border-gray-200 rounded-2xl mb-6 group-hover:bg-gray-900 group-hover:text-white transition-all duration-500 shadow-sm group-hover:shadow-xl group-hover:scale-110">
      ...
      {sections.about && (
        <section id="about" className="py-24 relative overflow-hidden bg-white">
            ...
            <div className="lg:grid lg:grid-cols-12 lg:gap-20 items-center">
                <motion.div 
                className="lg:col-span-6 mb-16 lg:mb-0 relative z-10"
                initial="hidden"
                whileInView="visible"
                ...
                <motion.div variants={fadeInUp} className="flex items-center gap-3 mb-8">
                    ...
                    <span className="text-primary-600 font-bold uppercase tracking-[0.2em] text-xs">{t.home.about.subtitle}</span>
                </motion.div>
                
                <motion.h2 variants={textReveal} className="text-5xl md:text-7xl font-serif font-black text-gray-900 mb-8 leading-[1]">
                    {t.home.about.titleFirst} <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-primary-400">{t.home.about.titleSecond}</span>
                </motion.h2>
                ...
                <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row sm:items-center gap-8">
                    <Link to="/amenities" className="bg-gray-900 text-white px-10 py-5 rounded-full font-bold transition-all hover:bg-primary-600 hover:shadow-xl hover:-translate-y-1 text-center sm:text-left shadow-lg">
                        {t.home.about.btn}
                    </Link>
                </motion.div>
                </motion.div>
                ...
                <div className="lg:col-span-6 relative h-[650px] hidden lg:block">
                    {/* Image 1: Slides down-left from top-right with scale */}
                    <motion.div 
                        style={{ y: yAbout1 }}
                        ...
                        className="absolute top-0 right-0 w-3/4 h-3/4 rounded-[3rem] overflow-hidden shadow-2xl z-0 transition-transform duration-700 origin-top-right group"
                    >
                         {/* Internal image zoom-out */}
                         <motion.div 
                            ...
                            whileInView="visible"
                         >
                            <img src={content.aboutImage1} alt="Exterior" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                            <div className="absolute inset-0 bg-primary-900/10 mix-blend-multiply"></div>
                         </motion.div>
                    </motion.div>
                    
                    {/* Image 2: Pops up with rotation correction */}
                    <motion.div 
                        style={{ y: yAbout2 }}
                        initial={{ opacity: 0, y: 100, x: -50, scale: 0.9, rotate: -10 }}
                        whileInView={{ opacity: 1, y: 0, x: 0, scale: 1, rotate: -3 }}
                        whileHover={{ rotate: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 1, delay: 0.2, type: "spring", bounce: 0.4 }}
                        className="absolute bottom-0 left-0 w-3/5 h-3/5 rounded-[3rem] overflow-hidden shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] z-10 border-[10px] border-gray-50 transition-transform duration-700 origin-bottom-left group"
                    >
                         {/* Internal image zoom-out */}
                         <motion.div 
                            className="w-full h-full"
                         ...
                         >
                            <img src={content.aboutImage2} alt="Interior" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                         </motion.div>
                    </motion.div>
                    ...
                    <motion.div 
                        ...
                        className="absolute -bottom-20 -right-20 w-64 h-64 bg-primary-200/50 rounded-full blur-3xl -z-10 animate-blob" 
                    />
                </div>
                ...
                <div className="lg:hidden mt-8 grid grid-cols-2 gap-4">
                    <motion.img 
                        ...
                        whileHover={{ scale: 1.05 }}
                        viewport={{ once: true }}
                        src={content.aboutImage1} alt="Exterior" className="w-full h-48 md:h-64 object-cover rounded-3xl shadow-lg" 
                    />
                    <motion.img 
                        initial={{ opacity: 0, x: 20, scale: 0.9 }}
                        whileInView={{ opacity: 1, x: 0, scale: 1 }}
                        ...
                        whileHover={{ scale: 1.05 }}
                        viewport={{ once: true }}
                        src={content.aboutImage2} alt="Interior" className="w-full h-48 md:h-64 object-cover rounded-3xl shadow-lg mt-8" 
                    />
                </div>
            </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                    <motion.div
                        ...
                        className="max-w-2xl"
                    >
                        <motion.span variants={fadeInUp} className="text-primary-500 font-bold tracking-[0.2em] uppercase text-xs mb-4 block">
                            {t.home.gallery.subtitle}
                        </motion.span>
                        <motion.h2 variants={textReveal} className="text-4xl md:text-6xl font-serif font-black mb-4">
                            {t.home.gallery.titleFirst} <br/> <span className="text-white/50">{t.home.gallery.titleSecond}</span>
                        </motion.h2>
                    </motion.div>
                    
                    <motion.a 
                        href="https://www.instagram.com/hotel_elarich/" 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="flex items-center gap-3 px-6 py-3 rounded-full border border-white/20 hover:border-primary-500 hover:bg-white/5 transition-all group"
                        initial={{ opacity: 0, x: 20 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: 0.5, duration: 0.8 }}
                    >
                        <Instagram size={18} className="group-hover:text-primary-500 transition-colors" /> 
                        <span className="font-bold text-sm tracking-wide">{t.home.gallery.follow}</span>
                    </motion.a>
                </div>

                {/* FEATURED VIDEO TOUR SECTION (Only renders if a video card exists) */}
                {videoItems.length > 0 && (
                    <div className="mb-20">
                        {videoItems.map(item => (
                            <motion.div 
                                ...
                                className="w-full"
                            >
                                <div className="flex items-center gap-6 mb-8 w-full max-w-4xl mx-auto text-center justify-center">
                                    <div className="h-px bg-gradient-to-r from-transparent to-white/30 flex-grow hidden md:block"></div>
                                    <h3 className="text-2xl md:text-3xl font-serif text-white uppercase tracking-widest leading-relaxed px-4">
                                        {t.home.gallery.tourTitle}
                                    </h3>
                                    <div className="h-px bg-gradient-to-r from-white/30 to-transparent flex-grow hidden md:block"></div>
                                </div>
                ...
                {/* DYNAMIC CARD LAYOUT (Images Only) */}
                <motion.div 
                    initial="hidden"
                    whileInView="visible"
                    viewport={{ once: true, margin: "-50px" }}
      ...
      {sections.cta && (
        <section ref={ctaRef} className="relative py-32 md:py-48 flex items-center justify-center overflow-hidden">
            <div className="absolute inset-0 z-0 overflow-hidden">
                <motion.img 
                    style={{ y: yCta, scale: 1.1 }}
                    src={content.ctaImage} 
                    className="w-full h-full object-cover filter brightness-50" 
                    alt="Espace Famille" 
                    loading="lazy" 
                />
                <div className="absolute inset-0 bg-gradient-to-r from-primary-900/90 to-black/50 mix-blend-multiply"></div>
            </div>
            ...
            <h2 className="text-5xl md:text-8xl font-serif font-black text-white mb-8 leading-[1.1] drop-shadow-2xl">
                {t.home.cta.titleFirst} <br/> <span className="text-primary-500">{t.home.cta.titleSecond}</span>
            </h2>
            <p className="text-xl md:text-2xl text-white/90 mb-12 font-medium max-w-2xl mx-auto">
                {t.home.cta.desc}
            </p>
            <Link to="/contact" className="bg-white text-primary-900 px-12 md:px-16 py-6 rounded-full font-black text-lg hover:bg-primary-500 hover:text-white transition-all shadow-[0_30px_60px_rgba(0,0,0,0.5)] transform hover:-translate-y-2 inline-flex items-center gap-3">
                {t.home.cta.btn} <ArrowRight size={24} />
            </Link>
            </motion.div>
        </section>
      )}
